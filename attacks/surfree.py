...
class SurFree():
    ...
    def attack(self, loss_func, x, y, init=None, buffer=None,
        ...
        if init==None:
            self.best_advs = get_init_with_noise(loss_func, x, y)
        ...
        # Check if X are already adversarials.
        self._images_finished = loss_func(x).argmax(1) != y
        ...
        if self.quantification:
            self.best_advs = self._quantify(self.best_advs)
        logits_best = loss_func(self.best_advs)
        query_cnt = self.get_nqueries()
        int_value = int(query_cnt[0].item())
        if int_value < self._max_queries:
            success = True
        return {
            'success': success,
            'query_cnt': int_value,

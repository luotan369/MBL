...
def attack():
    ...
    for _i in range(len(image_buffer.clean_images)):  
        ...
        with open('pertution.txt','a') as f:
            f.write(str(perturbation))
        perturbation_sign = torch.sign(perturbation)
        new_perturbation = perturbation_sign * args.linf
        ...
        if not args.test_fasr and not success:
            ...
            elif args.attack_method in ['surfree']:
               
                attack_output = attacker.attack(T, images, labels, init=(adv_images+new_perturbation).clip(0,1),
                                                buffer=adv_buffer) 
            else:
                attack_output = attacker.attack(loss_function, images, labels,init=perturbation ,
                                                buffer=adv_buffer)  

...
try:
    ...
    from surro_models.cifar10_models.preact_resnet import PreActResNet101
except Exception:
    pass

try:
    from surro_models.mnist_models.resnet import ResNet18 as ResNet18_mn
    from surro_models.mnist_models.vgg import VGG as VGG_mn
    from surro_models.mnist_models.pyramidnet import pyramid_net110 as pyramid_net110_mn
    from surro_models.mnist_models.densenet import DenseNet121 as DenseNet121_mn
    from surro_models.mnist_models.preact_resnet import PreActResNet101 as PreActResNet101_mn
except Exception:
    pass
...
def load_cifar_model(model_name, home_path='checkpoints/cifar10_target_models/', require_optim=False, defence_method=None):
    ...
    if model_name == 'resnet18':
        ...
        pretrained_model = pyramid_net110()
        model_checkpoint_path = os.path.join(home_path, 'Pyramidnet110_ckpt.t7')
    elif model_name == 'densenet':
        pretrained_model = DenseNet121()
        model_checkpoint_path = os.path.join(home_path, 'DenseNet_ckpt.t7')
    elif model_name == 'densenet121':
        pretrained_model = DenseNet121()
        model_checkpoint_path = os.path.join(home_path, 'Densenet121_ckpt.t7')
    elif model_name == 'preactresnet101':
        pretrained_model = PreActResNet101()
        model_checkpoint_path = os.path.join(home_path, 'PreactResnet101_ckpt.t7')
        ...
        raise NotImplementedError
    checkpoint = torch.load(model_checkpoint_path)  # todo debug
    try:
...
def load_mnist_model(model_name, home_path='checkpoints/mnist_target_models/', require_optim=False, defence_method=None):
    print('Load mnist model: ', model_name)
    if model_name == 'resnet18':
        pretrained_model = ResNet18_mn()
        model_checkpoint_path = os.path.join(home_path, 'Resnet18_ckpt.t7')
    elif model_name == 'vgg19':
        pretrained_model = VGG_mn('VGG19')
        model_checkpoint_path = os.path.join(home_path, 'VGG19_ckpt.t7')
    elif model_name == 'pyramidnet':
        pretrained_model = pyramid_net110_mn()
        model_checkpoint_path = os.path.join(home_path, 'PyramidNet_ckpt.t7')
    elif model_name == 'pyramidnet110':
        pretrained_model = pyramid_net110_mn()
        model_checkpoint_path = os.path.join(home_path, 'Pyramidnet110_ckpt.t7')
    elif model_name == 'densenet':
        pretrained_model = DenseNet121_mn()
        model_checkpoint_path = os.path.join(home_path, 'DenseNet_ckpt.t7')
    elif model_name == 'densenet121':
        pretrained_model = DenseNet121_mn()
        model_checkpoint_path = os.path.join(home_path, 'Densenet121_ckpt.t7')
    elif model_name == 'preactresnet101':
        pretrained_model = PreActResNet101_mn()
        model_checkpoint_path = os.path.join(home_path, 'PreactResnet101_ckpt.t7')
    else:
        raise NotImplementedError
    checkpoint = torch.load(model_checkpoint_path)  # todo debug

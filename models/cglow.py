...
class CondGlowStep(nn.Module):
    ...
    def forward(self, x, y, logdet=None, reverse=False):
        ...
        if reverse is False:
            # 1. cond-actnorm
            with open('testlog','a')as file:
                file.write("\n")
                file.write(f"x:{x.shape},y:{y.shape}")
...
class CondGlow(nn.Module):
    ...
    def __init__(self, x_size, y_size, x_hidden_channels, x_hidden_size, y_hidden_channels, K, L, down_sample_x,
                 down_sample_y):
        with open('testlog','a') as file:
            file.write(f"{x_size},{y_size},{x_hidden_channels},{x_hidden_size},{y_hidden_channels},{K},{L},{down_sample_x},{down_sample_y}")
    ...
    def encode(self, x, y, logdet=0.0, return_z=False):
        ...
        for layer, shape in zip(self.layers, self.output_shapes):
            if isinstance(layer, modules.Split2d):
                ...
                #  x:torch.Size([10, 1, 28, 28]),y:torch.Size([10, 4, 14, 14]),logdet:torch.Size([10])
                # x:torch.Size([10, 3, 32, 32]),y:torch.Size([10, 12, 16, 16]),logdet:torch.Size([10])

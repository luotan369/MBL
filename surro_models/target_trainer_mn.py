...
from mnist_models.resnet import ResNet18, ResNet34
from mnist_models.vgg import VGG
from mnist_models.pyramidnet import pyramid_net110
from mnist_models.densenet import DenseNet121
from mnist_models.preact_resnet import PreActResNet101
...
def prepare_normed_data():
    ...
    trainset = torchvision.datasets.MNIST(root=path, train=True, download=False, transform=transform_train)

    trainloader = torch.utils.data.DataLoader(trainset, batch_size=128, num_workers=2)

    testset = torchvision.datasets.MNIST(root=path, train=False, download=False, transform=transform_test)
...
def save(model_name, net, acc, epoch):
    ...
    save_path = '../checkpoints/mnist_target_models/{}_ckpt.t7'.format(model_name)
...
def save_normed(model_name, net, acc, epoch):
    ...
    if not os.path.isdir('mnist'):
        os.mkdir('mnist')
    save_path = '../checkpoints/mnist_target_models/{}_normed_ckpt.t7'.format(model_name)
    print('save path', save_path)
    if torch.__version__ == '1.3.0':
        torch.save(state, save_path)
    else:
...
if __name__ == '__main__':
    ...
    path = '../data/MNIST'
    main()
# python target_trainer_mn.py --model_name  Resnet18/VGG19/Pyramidnet110/Densenet121/PreactResnet101